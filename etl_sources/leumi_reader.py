...
def load_leumi_tables(folder: str | Path, recursive: bool = False) -> list[LoadedTable]:
    """
    Leumi-specific loading rule (tryout v1):
    1) read using pd.read_html
    2) use tables[1]
    """
    loaded: list[LoadedTable] = []
    for file_path in list_tabular_files(folder=folder, recursive=recursive):
        df = read_leumi_file(file_path)
        loaded.append(LoadedTable(path=file_path, extension=file_path.suffix.lower(), dataframe=df))
    return loaded


def read_leumi_file(path: str | Path) -> pd.DataFrame:
    file_path = Path(path)
    tables = pd.read_html(file_path)

    if len(tables) <= 1:
        raise ValueError(
            f"leumi rule expected tables[1], but found {len(tables)} table(s) in {file_path.name}"
        )
    ...
    df["__source_rule"] = "leumi_read_html_table_1"
    return df
